      ...
      <div className="hsc-container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredItems.map((item) => {
            const translatedItemInfo =
              t.gallery.items[item.id as unknown as keyof typeof t.gallery.items] || {
                title: item.title,
                subtitle: item.subtitle,
              };

            const translatedCategoryLabel =
              t.gallery.categories[item.category as keyof typeof t.gallery.categories] ||
              item.category;
            ...
            return (
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[rgba(42,33,26,0.85)] via-[rgba(0,0,0,0)] to-transparent flex flex-col justify-end p-4 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  <div className="text-sm font-bold">{translatedItemInfo.title}</div>
                  <div className="text-xs text-[#F0E4D3] capitalize">{translatedCategoryLabel}</div>
                </div>
              </div>
            );
          })}
